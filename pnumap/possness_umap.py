...
import pnumap.distances as dist


import pnumap.sparse as sparse

from pnumap.utils import (
    submatrix,
    ts,
    csr_unique,
    fast_knn_indices,
)
from pnumap.spectral import spectral_layout, tswspectral_layout
from pnumap.layouts import (
...
@numba.njit(
 ...
 # Possibility-Theory-Based rho, sigma, π, N with tunable α, β
def smooth_knn_dist_possibility(distances, k, local_connectivity=1.0, alpha=1.0, beta=1.0, sharpness=1.0):
    
    n_samples, n_neighbors = distances.shape
    sigmas = np.zeros(n_samples, dtype=np.float32)
    rhos = np.zeros(n_samples, dtype=np.float32)
    possibilities = np.zeros((n_samples, n_neighbors), dtype=np.float32)
    necessities = np.zeros((n_samples, n_neighbors), dtype=np.float32)

    for i in range(n_samples):
        ith_distances = distances[i]
        sorted_indices = np.argsort(ith_distances)
        sorted_dists = ith_distances[sorted_indices]

        # Compute rho (distance to the local connectivity neighbor)
        if sorted_dists.shape[0] >= local_connectivity:
            index = int(np.floor(local_connectivity))
            interpolation = local_connectivity - index
            if index > 0:
                rho = sorted_dists[index - 1]
                if interpolation > 0:
                    rho += interpolation * (sorted_dists[index] - sorted_dists[index - 1])
            else:
                rho = interpolation * sorted_dists[0]
            rhos[i] = rho
        else:
            rhos[i] = np.min(sorted_dists)

        max_dist = sorted_dists[-1]
        sigma = max(max_dist - rhos[i], 1e-5)  # prevent div-by-zero
        sigmas[i] = sigma

        for j, idx in enumerate(sorted_indices):
            dist = ith_distances[idx]

            # Tunable Possibility
            base_pi = np.maximum((dist - rhos[i]) / sigma, 0.0)
            pi_power = base_pi ** alpha
            pi_clipped = max(min(pi_power, 1.0), 0.0)
            pi = 1.0 - pi_clipped
            possibilities[i, idx] = pi

            # Tunable Necessity
            base_ne = np.maximum((max_dist - dist) / sigma, 0.0)
            ne_power = base_ne ** beta
            ne_clipped = max(min(ne_power, 1.0), 0.0)
            ne = 1.0 - ne_clipped
            necessities[i, idx] = ne

    return sigmas, rhos, possibilities, necessities
...
#  Hybrid Confidence gate (sigmoid)
# Hybrid Confidence gate (sigmoid) with decay clipping
def compute_membership_strengths_possibility(
    knn_indices,
    knn_dists,
    possibilities,
    necessities,
    ...
    alpha=1.0,
    beta=1.0,
    sharpness=1.0,       # λ: controls sharpness of sigmoid
    decay_clip=1e-3,     # Lower bound to prevent collapses
    normalize=True       # Optional per-row normalization
):
    """
    Construct sparse graph using hybrid PossUMAP formula:
    s_ij = sigmoid(pi + ne - 1) * exp(-(d_ij - rho_i) / sigma_i)

    Parameters:
    - knn_indices: shape (n_samples, n_neighbors)
    - knn_dists: shape (n_samples, n_neighbors)
    - possibilities: precomputed π_ij
    - necessities: precomputed N_ij
    - sigmas: per-sample decay scales
    - rhos: per-sample offsets
    - sharpness: sigmoid λ
    - decay_clip: minimum decay to avoid 0 edge weights
    - normalize: whether to normalize weights per row

    Returns:
    - rows, cols, vals [, dists]: COO-form sparse graph values
    """
    ...
    for i in range(n_samples):
        for j in range(n_neighbors):
            ...
            pi = possibilities[i, j]
            ne = necessities[i, j]
            dist_ij = knn_dists[i, j]

            # Confidence gate (sigmoid-based trust)
            # raw_input = sharpness * (pi + ne - 1.0)
            # input_gate = max(min(raw_input, 20.0), -20.0)
            # gate = 1.0 / (1.0 + np.exp(-input_gate))
            gate = stable_sigmoid(sharpness * (pi + ne - 1.0))


            # UMAP-style exponential decay with floor
            decay = np.exp(-np.maximum(dist_ij - rhos[i], 0.0) / (sigmas[i] + 1e-8))
            decay = np.maximum(decay, decay_clip)
        ...
        if normalize:
            row_start = i * n_neighbors
            row_end = (i + 1) * n_neighbors
            row_sum = np.sum(vals[row_start:row_end])
            if row_sum > 0:
                vals[row_start:row_end] /= row_sum

    # return (rows, cols, vals, dists) if return_dists else (rows, cols, vals)
    return rows, cols, vals, dists


## ## Possibility Score and Necessity Score  


# def fuzzy_simplicial_set_possibility(
#     X,
#     n_neighbors,
#     random_state,
#     metric,
...
# ):
#     """
#     Construct a fuzzy simplicial set using Possibility Theory.
...
#         Input data.
#     n_neighbors : int
#         Number of neighbors to use in local neighborhood.
...
#     metric_kwds : dict
#         Metric parameters.
#     knn_indices, knn_dists : arrays or None
#         Optionally precomputed k-nearest neighbor graph.
#     angular : bool
#         Use angular metric for RP trees.
#     set_op_mix_ratio : float
#         Mix ratio between union and intersection.
#     local_connectivity : float
#         Minimum local connectivity (rho baseline).
#     apply_set_operations : bool
#         Whether to apply fuzzy set combination.
#     verbose : bool
#         Print progress.
#     return_dists : bool
#         Whether to return sparse distance matrix.
    
#     Returns
#     -------
#     graph : scipy.sparse.coo_matrix
#         Fuzzy simplicial graph using possibility + necessity.
#     sigmas : np.ndarray
#         Scaling parameters per point.
#     rhos : np.ndarray
#         Local connectivity per point.
#     dists (optional) : sparse matrix of distances
#     """
...
#     # Step 1: Compute sigmas, rhos, possibility, and necessity scores
#     sigmas, rhos, possibilities, necessities = smooth_knn_dist_possibility(
#         knn_dists,
#         float(n_neighbors),
#         local_connectivity=float(local_connectivity),
#     )

#     # Step 2: Compute membership strengths (sparse structure)
#     rows, cols, vals, dists = compute_membership_strengths_possibility(
#     knn_indices, knn_dists, 
#     possibilities, necessities,
#     sigmas, rhos, return_dists=True)
...
#     # Step 4: Fuzzy set operations (fuzzy union/intersection)
#     if apply_set_operations:
#         graph_transpose = graph.transpose()
#         prod_matrix = graph.multiply(graph_transpose)

#         graph = (
#             set_op_mix_ratio * (graph + graph_transpose - prod_matrix)
...
#         return graph, sigmas, rhos, dists
...
def fuzzy_simplicial_set_possibility(
    ...
    return_dists : bool
        Whether to return sparse distance matrix.
    alpha, beta, sharpness : float
        Parameters controlling membership strength.
...
def simplicial_set_embedding(
    ...
    if output_dens:
        ...
        emb_graph, emb_sigmas, emb_rhos, emb_dists = fuzzy_simplicial_set_possibility(
            embedding,
            densmap_kwds["n_neighbors"],
            random_state,
            "euclidean",
...
class PossNessUMAP(BaseEstimator, ClassNamePrefixFeaturesOutMixin):
    ...
    def __init__(
        ...
        self.a = a
        self.b = b
        self.alpha = alpha  ## changed
        self.beta = beta    ## changed
        self.sharpness = sharpness   ## changed
        
        # For internal use
        self._a = None
        self._b = None
        self._sharpness = sharpness
        self._input_distance_func = None
        self._output_distance_func = None
    ...
    def fit(self, X, y=None, ensure_all_finite=True, **kwargs):
        ...
        **kwargs : optional
        ...
        if self.metric in ("bit_hamming", "bit_jaccard"):
            ...
            # )
            X = check_array(X, dtype=np.uint8, order="C")
        ...
        if self.metric == "precomputed" and self._sparse_data:
            ...
            (
                ...
                self._rhos,
                self.graph_dists_,
            ) = fuzzy_simplicial_set_possibility(
                X[index],
                self.n_neighbors,
                random_state,
                "precomputed",
    ...
    def transform(self, X, ensure_all_finite=True):
        ...
        # sigmas, rhos = smooth_knn_dist_possibility(
        #     dists,
        ...
        rows, cols, vals, dists = compute_membership_strengths_possibility(indices, 
               dists, possibilities, sigmas, rhos, necessities, return_dists=True )
    ...
    def update(self, X, ensure_all_finite=True):
        ...
        if self._small_data:
            ...
            if self._raw_data.shape[0] < 4096:
                ...
                self.graph_, self._sigmas, self._rhos = fuzzy_simplicial_set_possibility(
                    dmat,
                    self._n_neighbors,
                    random_state,
                    "precomputed",
...
def fuzzy_simplicial_set(
    ...
    result = fuzzy_simplicial_set_possibility(
        X,
        n_neighbors,
        random_state,
        metric,
    ...
    # Always return only 3 values to maintain compatibility with Scanpy
    if return_dists:
        return result[0], result[1], result[2]  # graph, sigmas, rhos
